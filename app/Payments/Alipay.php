...
class Alipay
{
        ...
        return [
            ...
            'config' => [
                ...
                'notify_url'=> [
                    'name' => 'notify_url',
                    'type' => 'text',
                    'note' => '回调域名，结尾不要带/，例如：https://alipay.com',
                ],
                'return_url'=> [
                    'name' => 'return_url',
                    'type' => 'text',
                    'note' => '跳转域名，结尾不要带/，例如：https://alipay.com',
                ],
            ]
        ];
    }
    ...
    public function purchase($order)
    {
        ...
        if ($this->config['notify_url']) {
            $urls = parse_url($order->notify_url);
            $order->notify_url = $this->config['notify_url'] . ($urls['path'] ?? '');
        }
        if ($this->config['return_url']) {
            $urls = parse_url($order->return_url);
            $order->return_url = $this->config['return_url'] . ($urls['path'] ?? '');
        }
        $config = [
        ...
        // Cache::tags('alipay')->put($order->order_sn, $result, now()->addMinutes(20));
        echo $result;
        die;
    }
